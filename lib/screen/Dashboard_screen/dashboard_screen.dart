...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  bool _hasInitialFetch = false; // Add this flag
  String _cachedEmployeeId = '';
  bool _isLoadingEmployeeId = false;

  String? _getEmployeeFilter() {
    ...
    final empId = auth.userData?['employee_id']?.toString() ??
        auth.userData?['emp_id']?.toString() ?? '';
    return empId.isNotEmpty ? empId : null;
  }
  ...
  Future<void> _initializeApp() async {
    ...
    Future<String> getEmployeeIdDirectly() async {
      final prefs = await SharedPreferences.getInstance();
      final savedId = prefs.getString('employee_id');
      print('📦 Direct from SharedPreferences: "$savedId"');
      return savedId ?? '';
    }
            ...
            IconButton(
              ...
              onPressed: () {
                ...
                if (isAdmin) {
                  ...
                  final nonAdminProvider = context.read<NonAdminDashboardProvider>();
                  nonAdminProvider.refreshDashboardData();
                }
              },
            ),
        ],
  ...
  List<Widget> _getScreens(PermissionProvider p) {
    ...
    screens.add(const MonthlyAttendanceScreen());
  ...
  Widget _buildHomeScreen() {
    ...
    if (isAdmin) {
      ...
      return _buildNonAdminHomeScreen();
    }
  }
  ...
  Widget _buildNonAdminHomeScreen() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!_isLoadingEmployeeId && _cachedEmployeeId.isEmpty) {
        ...
        if (savedId != null && savedId.isNotEmpty && savedId != '15') {
          print('✅ Found employee_id in SharedPreferences: $savedId');

          // Update auth userData if possible
          if (auth.userData != null && mounted) {
            auth.userData!['employee_id'] = savedId;
          ...
          if (!chartProvider.isLoading && chartProvider.chartData == null && mounted) {
            print('🔄 Fetching chart data for employee: $savedId');
            chartProvider.fetchEmployeeAttendanceChart(
              employeeId: savedId,
              month: currentMonth,
              context: context,
            );
    ...
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          children: [
            _buildNonAdminWelcomeHeader(auth),
            _buildNonAdminMonthDisplay(currentMonth),
            ...
            Padding(
              ...
              child: provider.isLoading && !provider.isRefreshing
                  ? _buildLoadingCards()
                  : provider.error != null
                  ? _buildNonAdminErrorWidget(provider)
                  : _buildNonAdminStatsGrid(provider, size, currentMonth),
            ),

            const SizedBox(height: 24),

            _buildChartSection(chartProvider, size, _cachedEmployeeId, formattedMonth),
...
// Add this new method to handle chart display
  Widget _buildChartSection(
      AttendanceChartProvider provider,
      Size size,
      String employeeId,
      String formattedMonth,
      ) {
    print('📊 Building chart section with employeeId: "$employeeId"');

    if (_isLoadingEmployeeId) {
      return _buildBarChartLoading();
    }
    ...
    if (provider.error != null) {
      return _buildBarChartError(provider, employeeId);
    }

    if (provider.chartData == null) {
      return _buildBarChartEmpty();
    }

    // Always show the chart widget, even with zero data
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: AttendanceBarChartWidget(
        chartData: provider.chartData!,
        isTablet: size.width > 600,
      ),
    );
  }
// Add this new method for the bar chart (unique name)
  Widget _buildAttendanceBarChart(
    ...
    if (provider.isLoading) {
      return _buildBarChartLoading(); // Changed name
    }

    if (provider.error != null) {
      return _buildBarChartError(provider, employeeId); // Changed name
    }

    if (provider.chartData == null || !provider.chartData!.hasData) {
      return _buildBarChartEmpty(); // Changed name
    }
...
// Loading widget with unique name
  Widget _buildBarChartLoading() {
    return Container(
...
// Error widget with unique name
  Widget _buildBarChartError(AttendanceChartProvider provider, String employeeId) {
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 16),
      ...
      child: Column(
        children: [
          ...
          Text(
            provider.error ?? 'Failed to load chart data',
...
// Empty widget with unique name
  Widget _buildBarChartEmpty() {
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 16),
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  _formatMonthForDisplay(currentMonth),
                  style: const TextStyle(
                    fontSize: 14,
                    fontWeight: FontWeight.w600,
  ...
  String _formatMonthForDisplay(String monthString) {
    try {
  ...
  Widget _buildNonAdminErrorWidget(NonAdminDashboardProvider provider) {
    return Container(
      padding: const EdgeInsets.all(20),
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 16),
          Text(
            provider.error ?? 'Failed to load your attendance data',
            textAlign: TextAlign.center,
            style: const TextStyle(color: Colors.grey, fontSize: 14),
          ),
          const SizedBox(height: 16),
  ...
  Widget _buildNonAdminStatsGrid(NonAdminDashboardProvider provider, Size size, String currentMonth) {
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        'No attendance records for ${_formatMonthForDisplay(currentMonth)}',
                        style: TextStyle(
        ...
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              fontSize: 22,
              fontWeight: FontWeight.w700,
              color: color == Colors.grey ? Colors.grey[600] : Colors.black87,
            ),
          ),
          if (subtitle != null) ...[
            const SizedBox(height: 2),
          ...
          Text(
            ...
            style: TextStyle(
              fontSize: 12,
              color: color == Colors.grey ? Colors.grey[500] : Colors.grey,
              fontWeight: FontWeight.w500,
            ),
          ),
        ],
  ...
  Widget _buildNonAdminSummaryItem({
    required IconData icon,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final dashboardProvider = Provider.of<DashboardSummaryProvider>(context, listen: false);
                final nonAdminProvider = Provider.of<NonAdminDashboardProvider>(context, listen: false);

                Navigator.of(dialogContext).pop();

                await auth.logout();
                await dashboardProvider.logout();
                await nonAdminProvider.logout();
