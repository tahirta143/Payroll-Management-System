...
// // Returns employee_id string if non-admin, else null (admin sees all)
...
//
//     // Read saved employee_id from SharedPreferences (already saved during login)
//     // We use sync read from AuthProvider userData
//     final auth = context.read<AuthProvider>();
//     final empId = auth.userData?['employee_id']?.toString() ??
//         auth.userData?['emp_id']?.toString() ?? '';
//     return empId.isNotEmpty ? empId : null;
//   }
//   // In your DashboardScreen's initState or where you set up providers
//
//   @override
//   void initState() {
//     super.initState();
...
//     // No need to initialize SharedPreferences separately as it's done in provider constructor
//
//     setState(() {
//       _isInitialized = true;
//     });
...
//         if (isAdmin) {
//           // Admin: Fetch admin dashboard data
...
//         } else {
//           // Non-Admin: Fetch employee-specific data
//           final nonAdminProvider = context.read<NonAdminDashboardProvider>();
//
//           // Get employee ID from auth data
//           final employeeId = authProvider.userData?['employee_id']?.toString() ??
//               authProvider.userData?['emp_id']?.toString() ??
//               '';
//
//           if (employeeId.isNotEmpty) {
//             nonAdminProvider.setEmployeeId(employeeId);
//             // Format current date to MM-YYYY for month parameter
...
//         actions: [
//           // Show avatar only for non-admin users
//           Builder(
//             builder: (context) {
...
//     final auth = context.watch<AuthProvider>(); // ADD THIS
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                Text(
                  'Employee ID: ${auth.userData?['employee_id'] ?? ''}',
                  style: const TextStyle(
                    color: Colors.white70,
                    fontSize: 14,
                  ),
